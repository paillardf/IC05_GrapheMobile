...
public final class DynamicUtilities {
    ...
    public static int getNodeCount(Graph graph, Interval timeInterval) {
        ...
        for (Node n : graph.getNodes()) {
            TimeInterval tnode = (TimeInterval) n.getNodeData().getAttributes().getValue(DynamicModel.TIMEINTERVAL_COLUMN);
    ...
    public static int getEdgeCount(Graph graph, Interval timeInterval) {
        ...
        for (Edge e : ((HierarchicalGraph) graph).getEdgesAndMetaEdges()) {
            TimeInterval tedge = (TimeInterval) e.getEdgeData().getAttributes().getValue(DynamicModel.TIMEINTERVAL_COLUMN);
            if (tedge.isInRange(timeInterval.getLow(), timeInterval.getHigh())) {

                TimeInterval tsource = (TimeInterval) e.getSource().getNodeData().getAttributes().getValue(DynamicModel.TIMEINTERVAL_COLUMN);
                TimeInterval tdest = (TimeInterval) e.getTarget().getNodeData().getAttributes().getValue(DynamicModel.TIMEINTERVAL_COLUMN);
